...
class ChannelDuplicate extends Component {
  constructor(props) {
    super(props);
    this.state = {
      currentUserID: [],
  ...
  isInputValid = ({ channelName, channelDetails }) => {
    if (channelName.length < 5 || channelDetails.length < 5) {
      this.setState({
        ...this.state,
        error: `Channel name & Channel description
                must exceed five(5) characters.
               `,
      });
  ...
  hasChannelUpdated = () => {
    ...
    channelsRef.once("value", (snap) => {
      ...
      this.setState({ channels: msgsArray.reverse() }, () =>
  ...
  checkSubscriberByID = () => {
    const channel_suscribers_id = this.props.currentChannel.suscribed_users
      .users_id;
  ...
  isUserSubscribed = (channel) => {
    ...
    if (channel !== null) {
      const channel_suscribers_id = channel.suscribed_users && channel.suscribed_users.users_id;
      const current_user_id = this.props.currentUser.uid;
      if (!!channel_suscribers_id && !!current_user_id) {
        if (channel_suscribers_id.includes(current_user_id)) {
          isSubscribed = true;
