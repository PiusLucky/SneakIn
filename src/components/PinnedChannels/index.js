...
import { SubButton, PinColor, TimeColorV2 } from "../Themefy/CustomTheme";
import WhiteColor from "../Themefy/WhiteColor";
import  { connect } from "react-redux";


const PinnedChannels = (props) => {
  const {
    type,
    changeChannel,
    getInitialActivity,
    ...
    >
      ...
      <div className="viewed_updates--left">
        <div className="add_status--header channel--header">
          <WhiteColor>
            {" "}
            {shortenFileName(channel_item.name, 30, 0.99, "...")}
          </WhiteColor>
          <TimeColorV2 as="p" className="channel--time" id={!isDesktop?"channel--time_mobile": null}>
        ...
        retrieveChannelSubFromId(channel_item.id)?"normal_opacity":"increase_opacity"}`}>
          <p className="channels-lastmsg" id={`${theme === "light"?"make_black":""}`} >
          ...
          <div className="add_flex_pin">
            <PinColor className="pin">
              <AiFillPushpin className="channel-pin" />
            </PinColor>
            {clearNotificationDesktop(channel_item) &&
...
export default connect(mapStateToProps)(PinnedChannels);
